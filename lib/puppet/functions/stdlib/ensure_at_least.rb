...
Puppet::Functions.create_function(:'stdlib::ensure_at_least') do
  ...
  def ensure_at_least(package, minversion, installversion = nil, provider = nil)
    ...
    when 1
      ...
      if provider.nil? or queryresult.first['provider'] == provider
    ...
    when 2..Float::INFINITY
      ...
      if queryresult.uniq {|e| e['provider']}.length == 1
        ...
        # We're dealing with multiple providers now and need to select just one
        # Only try to determine the default provider if it was not specified in the function call
        if provider.nil?
          ...
          call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Multiple Package Inventory results found and no provider given, attempting to auto-select provider")
          ...
          def_providers.each do |os,data|
            if (data['type'] == 'operatingsystem' and os == facts['operatingsystem'].downcase) or
               (data['type'] == 'osfamily' and os == facts['osfamily'].downcase)
              # Found a def_providers match for this OS/OS Family
              call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Auto-selecting for #{data['type']} #{os}")
              ...
              if data['filter']
                ...
                if found == false and defined?(def_provider) != nil
      ...
      call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Auto-selected provider #{provider}")
      ...
      when 0
        result = { 'version' => '' }
        call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | No packages found")
      when 1
        result = filtered_result.first
        call_function('notice',  "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Found version #{result['version']}")
      when 2..Float::INFINITY
        # We found multiple versions for this package, need to recursively compare them to find the highest version
        call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Found multiple versions, auto-selecting highest")
        highest = ''
        ...
        call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Auto-selected highest version #{highest}")
      else
        result = { 'version' => '' }
        call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Error occured finding packages")
      end
    else
      result = { 'version' => '' }
      call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Error occured finding packages")
    ...
    when 0, 1
      call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} | Provider #{provider} | Returned installed version #{installed}, as it was newer or same as minimum version #{minversion}")
      return installed
    else
      if installversion.nil?
        retversion = minversion
      else
        retversion = installversion
      end
      call_function('notice', "stdlib::ensure_at_least | Node: #{trusted['certname']} | Package: #{package} |#{unless provider.nil? then " Provider #{provider} |" end} Returned version to install: #{retversion}, as minimum version of #{minversion} was not installed")
      return retversion
    end
  end
end
